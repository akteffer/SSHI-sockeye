...
## Spoiler alert - results from Art
d_sw_results <-readRDS("data/sockeye_lw_results_mar18.rds")

## Read in data
#d <-read.csv("data/FULL_ONNE productivity infection analysis_stockspecific.csv")
...
plot(log(d_sw$Mass)~log(d_sw$Length)) 

## How many fish per year per stock?
t1 <- d_sw %>%
  group_by(Year, Stock) %>%
  tally() 
t2 <- data.frame(t1)
t3 <- xtabs(n~Stock+Year, data=t2)
t4 <- t3[rowSums(t3[,-1]) > 0, ]
t5 <- addmargins(t4)
write.table(t5, file = "Table_MD_samptotals.txt", sep = ",", quote = FALSE, row.names = T)
...
## Create a PDF
pdf(file="sock_lwr_mar18_21.pdf",width=11,height=8.5,paper="USr")
...
pathogen = stringr::str_detect(coef_names, "pa")
...
#saveRDS(sockeye_lw,"sockeye_lw_results_mar18.rds")
...
## Plotting and shit
d_sw_results <-readRDS("data/sockeye_lw_results_mar18.rds")
post_probs <- d_sw_results$post_probs
names(post_probs)
ss_load <- d_sw_results$ss_load
names(ss_load)
