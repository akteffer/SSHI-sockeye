...
jags_model = function() {
  ...
  for (q in 1:n_obs) {
    ...
    Mass_hat[q] <- b0 + b1*length[q] + b2[jaz[q]]*(pathogen[q])^pow + jaz_eff[jaz[q]] + yr_eff[yr[q]] #
...
jags_params = c("b0", "b1","b2","pow","B2", "sig_resid", "sig_jaz",  "Mass_hat", "residual","jaz_eff","yr_eff")
...
for(j in 1:length(mod_ind)){
  ...
  jags_model = function() {
    ...
    for (q in 1:n_obs) {
      Mass[q] ~ dnorm(Mass_hat[q], 1/sig_resid^2)
      Mass_hat[q] <- b0 + b1*length[q] + b2[jaz[q]]*(pathogen[q]) + jaz_eff[jaz[q]] + yr_eff[yr[q]] 
      residual[q] <- Mass[q] - Mass_hat[q]
    }
  }
  ...
  ### step 4: parameters to monitor ###
  jags_params = c("b0", "b1","b2","B2", "sig_resid", "sig_jaz",  "Mass_hat", "residual","jaz_eff","yr_eff")
...
#saveRDS(sockeye_lw,"sockeye_FW_lw_results_jan24_2023.rds")
